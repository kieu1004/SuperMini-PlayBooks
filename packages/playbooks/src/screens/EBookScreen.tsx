...
const EBookScreen = () => {
  ...
  //         const response = await fetch('http://149.28.134.133/api/v1/reader/books', {
  //           method: 'GET',
  //           headers: {
  //             'token': `${token}`,
  //           },
  ...
  //           console.log('Book List:', responseData.result);
  //         } else {
  //           console.error('Fetch Book List error:', response.status);
  //         }
    ...
    if (item.type == 'Ebook') {
      return (
        <Card mode="contained" onPress={() => navigation.navigate('EBookDetailScreen', { item })}>
          <Card.Cover source={{ uri: `${item.coverPhoto}?${index}` }} />
          <Card.Title
            titleVariant="titleMedium"
            subtitleVariant="bodyMedium"
            title={`${item.category} • ${truncatedTitle}`}
